/**
* copyright 2018-2021 by Neil Jackson
*
* adapted from
...
metadata {
	definition (
		name: "MimoLite io module as garage door controller", 
        ...
        //"java.lang.IllegalArgumentException: No enum constant hubitat.device.DataType.12-BIT UNSIGNED INTEGER"
...
//{ DEPRECATED versions of main_test_code()
	def mainTestCode_old3(){
		def message = ""
	...
	def mainTestCode_old1(){
		...
		// debugMessage += "(new hubitat.zwave.Command()): " + (new hubitat.zwave.Command()).format() + "\n";
		// try {debugMessage += (new hubitat.zwave.Command()).getProperties().toString() + "\n";} catch(e){debugMessage += e.message + "\n";}
		// try {debugMessage += (new hubitat.zwave.Command()).metaClass.methods*.name.sort().unique().toString()   + "\n";} catch(e){debugMessage += e.getMessage() + "\n";}
		...
		// spits out: class hubitat.zwave.commands.notificationv3.NotificationReport //see https://graph.api.smartthings.com/ide/doc/zwave-utils.html#notificationV3/notificationReport 
		...
		//spits out class hubitat.zwave.commands.alarmv1.AlarmReport //see https://graph.api.smartthings.com/ide/doc/zwave-utils.html#alarmV1/alarmReport
		...
		//spits out class hubitat.zwave.commands.alarmv2.AlarmReport  //see https://graph.api.smartthings.com/ide/doc/zwave-utils.html#alarmV2/alarmReport
		...
		debugMessage += "hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT: " + hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT + "\n";
		
		// sendHubCommand(
			// logZwaveCommandFromHubToDevice(
				// [
					// (new hubitat.zwave.commands.alarmv1.AlarmGet(alarmType: hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT)).format()
				// ]
			// ).collect{new hubitat.device.HubAction(it)}
		// );
		
		debugMessage +=  "(new hubitat.zwave.commands.alarmv1.AlarmGet(alarmType: hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT)) instanceof hubitat.zwave.Command: " + ((new hubitat.zwave.commands.alarmv1.AlarmGet(alarmType: hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT)) instanceof hubitat.zwave.Command) + "\n";
		...
		//spits out: cmd.getProperties()['class']: class hubitat.zwave.commands.sensorbinaryv2.SensorBinaryReport
		...
		debugMessage +=  "cmd.getProperties()['class']: " + cmd.getProperties()['class'] + "\n"; 
		//spits out: cmd.getProperties()['class']: class hubitat.zwave.commands.sensorbinaryv1.SensorBinaryReport  
		...
		debugMessage +=  "cmd.getProperties(): " + cmd.getProperties() + "\n"; 
		//spits out: cmd.getProperties(): [commandClassId:48, commandId:3, class:class hubitat.zwave.commands.sensorbinaryv1.SensorBinaryReport, SENSOR_VALUE_DETECTED_AN_EVENT:255, sensorValue:0, SENSOR_VALUE_IDLE:0, CMD:3003, payload:[0]]
		debugMessage +=  "cmd.format(): " + cmd.format() + "\n"; 
	...
	def mainTestCode_old2(){
		...
		// sendHubCommand(
			// logZwaveCommandFromHubToDevice(
				// [
					// new hubitat.zwave.commands.configurationv2.ConfigurationBulkGet(parameterOffset:1, numberOfParameters: 2),
					// //new hubitat.zwave.commands.configurationv2.ConfigurationGet(parameterNumber:3),
				// ]
			// ).collect{new hubitat.device.HubAction(it.format())}
		// );
		...
		// debugMessage += hubitat.device.DataType.getProperties().inspect() + "\n";
		// try{debugMessage += hubitat.device.DataType.INTEGER.toString() + "\n";}catch(e){debugMessage += e.toString() + "\n";  }
		// try{
			// Integer x = hubitat.device.DataType.INTEGER;
			
			// debugMessage += x + "\n";}catch(e){debugMessage += e.toString() + "\n";  }
		// debugMessage += now() + "\n";
		
		// try{debugMessage += "" + hubitat.device.DataType.INTEGER.getProperties() + "\n";}catch(e){debugMessage += e.toString() + "\n";  }
		// try{debugMessage += "" + hubitat.device.DataType.BOOLEAN + "\n";}catch(e){debugMessage += e.toString() + "\n";  }
...
//{ PLATFORM-REQUIRED LIFECYCLE FUNCTIONS (parse(), updated())
    ...
    def parse(String description) {
        ...
        // }
        //The original code from Todd Wackford ( see
        // https://github.com/SmartThingsCommunity/SmartThingsPublic/blob/dc5a17f016a1424694f820fc84cd57895aaa84ec/devicetypes/smartthings/mimolite-garage-door-controller.src/mimolite-garage-door-controller.groovy#L95
        // ) would do sendEvent(name: "powered", value: "powerOn") in response
        // to every incoming zwave command except those for which cmd.CMD ==
        // "7105" (i.e. commands of type
        // hubitat.zwave.commands.alarmv1.AlarmReport) I think the idea with
        // this scheme was that any non-alarm command indicates that the device
        // is powered and working.  The problem is that a "7105" command could
        // be a report that there is no alarm condition (which the device would
        // send in response to an AlarmGet command). also, in general, (although
        // possibly not with the mimolite io module), the alarm type might be
        // something other than POWER_MANAGEMENT. In general, it is not ideal to
        // set the powered attribute to powerOn in response to any non-alarm
        // command, because it might conceivably happen that the device would
        // send some regular reports even during a brownout condition, but the
        // thing that makes us want to set powered to powerOn in response to any
        // on-alarm command is that the device does not automatically send a
        // "power restored" alarm report upon first turning on after having been
        // powered off. therefore, in order to provide fast response to
        // regaining power, without having to wait for a scheduled poll, we will
        // regard any signal other than an "alarm is high" report that power is
        // OK.  /in practice, brownouts are unlikely - the more frequent case is
        // a complete loss of power followed by a complete regaining of power.
        
        
        if (cmd) {
            if(
                !(
                    cmd instanceof hubitat.zwave.commands.alarmv1.AlarmReport &&
                    cmd.alarmType == hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT &&
                    cmd.alarmLevel
                 )
            ) {
        ...
        // //the class of cmd is hubitat.zwave.commands.switchbinaryv1.SwitchBinaryReport
        ...
        // //the class of zwave.switchBinaryV1.switchBinaryReport(value:0) is hubitat.zwave.commands.switchbinaryv1.SwitchBinaryReport
...
//{  PARSING-HELPER FUNCTIONS
    ...
    def zwaveEvent(hubitat.zwave.commands.basicv1.BasicReport cmd) { //command class code: BASIC (0x20)
    ...
    def zwaveEvent(hubitat.zwave.commands.associationv2.AssociationReport cmd) { //command class code: ASSOCIATION (0x85)
    ...
    def zwaveEvent(hubitat.zwave.commands.associationv2.AssociationGroupingsReport cmd) { //command class code: ASSOCIATION (0x85) 
        //we are using association version 2 here even though the device only really supports association v1, due to the bug noted at https://community.smartthings.com/t/bug-in-z-wave-command-parser-found/9924 
    ...
    def zwaveEvent(hubitat.zwave.commands.basicv1.BasicSet cmd) { //command class code: BASIC (0x20)
    ...
    def zwaveEvent(hubitat.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd) { //command class code: SWITCH_BINARY (0x25)
    ...
    def zwaveEvent(hubitat.zwave.commands.sensorbinaryv1.SensorBinaryReport cmd) { // 'SENSOR_BINARY': 0x30,
    ...
    def zwaveEvent(hubitat.zwave.commands.alarmv1.AlarmReport cmd) { // 0x71: 'NOTIFICATION',
        ...
        def returnValue = [];
        if(cmd.alarmType == hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT)
        {
            if(cmd.alarmLevel) {
    ...
    def zwaveEvent (hubitat.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd) { // 0x31: 'SENSOR_MULTILEVEL',
...
//{  OUTBOUND SEQUENCES OF ZWAVE COMMANDS
    ...
    def getCommandsForConfigure() {
        return delayBetweenGood([
            ...
            // second of the above two commands undoes the first if you are
            // trying to change the triggerBetweenThresholds flag -- the
            // operation must be atomic, since both those flags are bits from
            // the same register  -- with all the overhead of zwave commands
            // (which is great because it makes the system for understandable
            // and well-behaved and flexible) -- Fortrezz ought not to have
            // implemented thes flags as two bits in the same one-byte
    ...
    def getCommandsForRefresh()  {
        return delayBetweenGood([
                ...
                (new hubitat.zwave.commands.sensorbinaryv1.SensorBinaryGet()).format(), //request a report of the (binary) state of the input
                (new hubitat.zwave.commands.alarmv1.AlarmGet(alarmType: hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT)).format(), //request a report of the low-voltage alarm state.
                (new hubitat.zwave.commands.meterpulsev1.MeterPulseGet()).format() //request a report of the meter pulse count.
...
//{  Z-WAVE LOGGING
    ...
    //the following two logZwaveCommand...() functions are pass-through
    //functions -- they will return exactly what is passed to them.  we will
    //wrap all outgoing and incoming zwave commands (in some reasonable format),
    //respectively, in these two functions. These functions accept as arguments
    //exactly the sort of thing that is allowed to be returned from a command
    //function (e.g. off(), on(), refresh(), etc.), namely, a string or an array
    //whose elements are strings (or the type of thing returned by delay())
    //unfortunately, whereas the commands constructed with, for instance,
    //zwave.basicV1.basicGet() produce a meaningful string in response to the
    //format() method, the object returned by zwave.parse(description) in the
    //parse() function behaves differently. therefore, I have resorted to a
    //rather hacky json serialize/deserialize process, so that the hubToDevice
    //commands that we log are of the same type as the deviceToHub commands.
    ...
    //x is expected to be anything that would be a suitable return value for a
    //command method - nameley a list of zwave commands and "delay nnn"
    //directives or a single zwave command. oops -- it is not instances of
    //hubitat.zwave.Command that are returned from command functions but
    //rather lists of strings that result from calling the format() method on
    //hubitat.zwave.Command objects. fortunately, zwave.parse(x.format())
    //will return something equivalent to x in the case where x is an instanceof
    //hubitat.zwave.Command.  So, if we record the format()'ed strings, we
    //won't be losing any information.
    
    //therefore, x is expected to be a string or a list of strings, and any
    //members that happen to be instances of hubitat.zwave.Command will be
    //converted into strings using hubitat.zwave.Command's format()
    //method.
    def logZwaveCommand(x, attributeName) {
        if(x)
        {
            ...
            //if we needed to distinguish between strings and zwave commands, we could do " ... instanceof hubitat.zwave.Command"
            sendEvent(
                name: attributeName, 
