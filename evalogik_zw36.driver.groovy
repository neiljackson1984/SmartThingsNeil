/**
 *  2023-02-13: Modified by Neil Jackson.  Modified the ```def
 *      zwaveEvent(hubitat.zwave.commands.supervisionv1.SupervisionGet, def)```
 *      function to perform a special action in the case where the command's
 *      sessionId field is 1.  This works around a weird condition that seems to
 *      befall the z-wave mdoule upon power-up.
 *
 *  Evalogik Smart Plug v1.0.0(HUBITAT)
 *
 *      Models: ZW36
 *
 *  Author: winnie (sky-nie)
 *
 *  Documentation:
 *
 *  Changelog:
 *
 *    1.0.0 (12/02/2022) - Initial Release
 *
 * Reference：
...
metadata {
    definition (
            name: "Evalogik Smart Plug(ZW36)",
            namespace: "sky-nie",
            author: "winnie",
            mnmn: "SmartThings",
            vid:"generic-switch",
            importUrl: "https://raw.githubusercontent.com/sky-nie/hubitat/main/evalogik/evalogik-smart-plug.groovy"
    ) {
        capability "Actuator"
        capability "Sensor"
        capability "Switch"
        ...
        fingerprint mfr: "0312", prod: "C000", deviceId: "C001", deviceJoinName: "Evalogik Plug", ocfDeviceType: "oic.d.smartplug"//, inClusters:"0x5E,0x25,0x70,0x5B,0x85,0x8E,0x59,0x55,0x86,0x72,0x5A,0x73,0x9F,0x6C,0x7A"    //ZW30  Evalogik Smart Switch
    }
...
def zwaveEvent(hubitat.zwave.commands.supervisionv1.SupervisionGet cmd, ep = null ) {
    ...
    if(cmd.sessionID == 1){
        logTrace "sending a follow-up switchBinaryGet because the session id of this supervisionGet is 1. "
        /** 
         * This is to work around a weird condition that occurs, reliably, on
         * power-up of the z-wave module, where the zwave module is unresponsive
         * to the first SwitchBinarySet command after power up.  I do not
         * understand exactly why this occurs, but I have noticed that:
         *
         *      1.  upon power-up, the zwave module sends a SupervisionGet
         *          command (which contains an encapsulated SwitchBinaryReport) to the hub.
         *
         *      2.  If, immediately after power-up of the z-wave module, we click the "refresh" button in 
         *          the hubitat web interface (which causes a SwitchBinaryGet to be sent 
         *          to the z-wave module), we can then 
         *          send a SwitchBinarySet and the z-wave module will obey it (whereas if
         *          we had simply sent the SwitchBinarySet command immediately after power-up, 
         *          the z-wave module wouild not obey it.
         *
         * Based on these two observations, I have invented this workaround,
         * which is to send a switchBinaryGet command to the z-wave module
         * whenever the module sends to the hub a SupervisionGet command having
         * sessionId==1.  sessionId being 1 is a reasonably-reliable indicator
         * that the supervisionGet command is happening as part of the z-wave
         * module's power-up behavior.  I beleive that the sessionId field of
         * the Supervision z-wave commands is an 8-bit field, and therefore,
         * probably, rolls back to 1 every 255'th Supervision command. Even in
         * the case where sessionId is 1 because the counter has rolled over
         * (rather than power-up), it should still be fairly harmless to send an
         * extra SwitchBinaryGet command to the z-wave module.
         */
        sendHubCommand(new hubitat.device.HubAction(secure(zwave.switchBinaryV1.switchBinaryGet(), ep), hubitat.device.Protocol.ZWAVE)) 
    }
...
private getPowerFailureRecoveryParam() {
    return getParam(3, "Power Failure Recovery", 1, 2, powerFailureRecoveryOptions)
}
...
private getBrightnessControlParam() {
    return getParam(7, "Brightness Control", 1, 2, brightnessControlOptions)
}
...
private static getBrightnessControlOptions() {
    return [
            "0":"Bright",
            "1":"Medium",
            "2":"Low"
