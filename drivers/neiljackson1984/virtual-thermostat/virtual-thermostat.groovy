...
metadata {
	definition (name: "Virtual Thermostat", namespace: "neiljackson1984", author: "Neil Jackson") {
        ...
        attribute("lastAlexaSetpointCommand", "string"); //this attribute records a bit of state that lets us detect and respond properly to the commands that alexa sends when you say "alexa, set the xxx thermostat to 67 degrees" (she sends a setHeatingSetpoint and a setCoolingSetpoint command in rapid succession, with the heating setpoint four degrees below the commanded setpoint and the cooling setpoint four degrees above the commanded setpoint.)
...
def runTheTestCode()
{
    ...
    def thisDevice;
    //thisDevice = device;
    thisDevice = getParent().getAllChildDevices()[0];
    //confusingly, "device.events()" and "getParent().getAllChildDevices()[0].events()" do not return the same thing. (see the note "As of now this object is a different type than the Device object available in SmartApps..." on https://docs.smartthings.com/en/latest/ref-docs/device-handler-ref.html#device 
    // the device object in the SmartApp seems to return usable events whereas the device object here in the device handler returns objects that we are forbidden from calling any methods on or accessing any properties of (if we try, an error occurs: 	java.lang.SecurityException: Getting properties on class physicalgraph.event.DefaultEvent is not allowed).
    def recentCommands = thisDevice.events().findAll{it.source == "COMMAND"};
    // def matches = thisDevice.events().collect{
        // [
            // it.date.format(preferredDateFormat, location.getTimeZone()),
            // it.description,
            // it.descriptionText,
            // it.value,
            // it.source,
        ...
        // ]
    // };
    // debugMessage += matches.toString() + "\n";
    
    // def matches = device.eventsSince(new Date(now() - 10000)).collect{
    // def matches = device.event.collect{
        // [
            // //it.getDate(),//?.format(preferredDateFormat, location.getTimeZone()),
            // //it.getDescription(),
            // //it.getValue(),
            // it.source
        // ]
    // }
    // // def matches = device.events().findAll{return true;}
...
// {
    ...
    // //confusingly, "device.events()" and "getParent().getAllChildDevices()[0].events()" do not return the same thing. (see the note "As of now this object is a different type than the Device object available in SmartApps..." on https://docs.smartthings.com/en/latest/ref-docs/device-handler-ref.html#device 
    // // the device object in the SmartApp seems to return usable events whereas the device object here in the device handler returns objects that we are forbidden from calling any methods on or accessing any properties of (if we try, an error occurs: 	java.lang.SecurityException: Getting properties on class physicalgraph.event.DefaultEvent is not allowed).
    // def recentCommands = thisDevice.events([max:1]).findAll{( (String) it.source) != "DEVICE"};
    // log.debug "recentCommands: " + recentCommands.collect{it ->  [ "date": it.date.format(preferredDateFormat, location.getTimeZone()),
            ...
            // "unit" : it.unit,
            // "inspect()" : it.inspect()
            // //"json form" : groovy.json.JsonOutput.toJson(it)
            // ]
    // };
...
def setHeatingSetpoint(x) {
    ...
    //look for a recent invokation of setCoolingSetpoint() within the last n seconds.
    def averagedSetpoint;
    def matches = getParent().getAllChildDevices()[0].events([max:1]).findAll{
        ( (String) it.source) == "APP_COMMAND"  && it.displayName == "Amazon Alexa" && it.value == "setHeatingSetpoint"
    };
    if(matches) //i.e. if this is a command from alexa
    {
        //look for a recent command from alexa commanding the opposite setpoint
        //we are relying on statesSince to return states in reverse chronoological order
        // if there has been an alexa command to setCoolingSetpoint in the last 3 seconds, then we will set the setpoint to the average of x and that requested cooling setpoing
        
        device.statesSince("lastAlexaSetpointCommand", new Date(now() - 3000)).find{it.jsonValue.commandName == "setCoolingSetpoint"}.each{
            averagedSetpoint = (x + it.jsonValue.value)/2; 
            log.debug "rapid succession of alexa setpoint commands detected -- averaging the setpoint to ${averagedSetpoint}";
        }
        
        sendEvent(
            name: "lastAlexaSetpointCommand",
            value: groovy.json.JsonOutput.toJson([commandName: matches[0].value, value: x, date: matches[0].date])
        ); 
    }
    setSetpoint(averagedSetpoint ?: x);
}
...
def setCoolingSetpoint(x) {
    ...
    //determine if this command is coming from Alexa.
    def matches = getParent().getAllChildDevices()[0].events([max:1]).findAll{
        ( (String) it.source) == "APP_COMMAND"  && it.displayName == "Amazon Alexa" && it.value == "setCoolingSetpoint"
    };
    if(matches) //i.e. if this is a command from alexa
    {
        //look for a recent command from alexa commanding the opposite setpoint
        ...
        device.statesSince("lastAlexaSetpointCommand", new Date(now() - 3000)).find{it.jsonValue.commandName == "setHeatingSetpoint"}.each{
            averagedSetpoint = (x + it.jsonValue.value)/2; 
            log.debug "rapid succession of alexa setpoint commands detected -- averaging the setpoint to ${averagedSetpoint}";
        }
...
def take()
{
  ...
  def params = [
          ...
          lineChartQuery(
          	[
                [
                    'name': 'setpoint',
                    'data': 
                        def setPointData = (listOfStates = unlimitedStatesBetween("setpoint",new Date(plotRangeX.min()),new Date(plotRangeX.max()),['includeLatestStatePriorToStartDate':true])).collect{ theState ->
                            [
    ...
    log.debug "setPointData: " + setPointData.collect{it -> [ new Date(it[0]).format(preferredDateFormat, location.getTimeZone()), it[1]]};
...
List unlimitedStatesBetween(String attributeName, Date startDate, Date endDate, Map options = [:])
{
    ...
    //log.debug "unlimitedStatesBetween(" + attributeName + ", " +  startDate.format(preferredDateFormat, location.getTimeZone()) + ", " + endDate.format(preferredDateFormat, location.getTimeZone()) + ", " + options + " is returning " + states;
    return states; 
}
